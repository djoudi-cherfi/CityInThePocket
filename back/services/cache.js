...
const cache = (duration = PEREMPTION) => async (req, res, next) => {
  ...
  if (await asyncClient.exists(urlKey)) {
    ...
    // attendez une minute ! quelle méthode est systématiquement utilisée par
    // tous les middlewares de controller et à laquelle on passe
    // systématiquement les données avec lesquelles on répond
